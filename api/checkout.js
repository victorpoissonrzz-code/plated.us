...
// Front-end contract:
//   POST /api/checkout
//   Body: { lineItems, customerEmail, customerName, shippingAddress, delivery, promoCode }
...
module.exports = async (req, res) => {
      ...
      try {
        ...
        // Stripe Checkout Sessions do NOT accept negative or zero unit_amount values.
        // Discount / promo line items (amount <= 0) must be excluded here — apply
        // discounts via Stripe Coupons / allow_promotion_codes instead.
        const stripeLineItems = lineItems
                .filter(item => Math.round(Number(item.amount) || 0) > 0)
                .map(item => ({
                            price_data: {
        ...
        if (stripeLineItems.length === 0) {
                  return res.status(400).json({ error: 'Cart contains no payable items.' });
        }

        // ── Promo code resolution ─────────────────────────────────────────────────
        // If a promo code is provided, look it up via the Stripe promotionCodes API.
        // If the code is invalid or expired, return a clear error to the front-end.
        let discounts = [];
        ...
        if (promoCode && promoCode.trim() !== '') {
                ...
                if (promoCodes.data.length === 0) {
                            return res.status(400).json({ error: `Promo code "${promoCode}" is invalid or expired.` });
                }

                resolvedPromoCode = promoCodes.data[0];
                  discounts = [{ promotion_code: resolvedPromoCode.id }];
        }
              // ─────────────────────────────────────────────────────────────────────────
        ...
        // Apply the resolved promo code as a Stripe discount.
        // NOTE: `discounts` and `allow_promotion_codes` are mutually exclusive in Stripe.
        // When a code is pre-validated we pass it directly; otherwise we let the
        // customer enter one themselves on the Stripe-hosted checkout page.
...
async function sendOrderNotification({
      ...
      const subtotal = lineItems.reduce((sum, i) => sum + (i.amount * i.quantity), 0) / 100;

  // Compute discount amount for the notification email
  let discountLabel  = 'None';
      let discountAmount = 0;
      if (resolvedPromoCode) {
              const coupon = resolvedPromoCode.coupon;
              if (coupon.amount_off) {
                        discountAmount = coupon.amount_off / 100;
                        discountLabel  = `${promoCode} (-$${discountAmount.toFixed(2)})`;
              } else if (coupon.percent_off) {
                        discountAmount = subtotal * (coupon.percent_off / 100);
                        discountLabel  = `${promoCode} (-${coupon.percent_off}% = -$${discountAmount.toFixed(2)})`;
              }
      }

  const total = Math.max(0, subtotal - discountAmount + (delivery.cost || 0));
