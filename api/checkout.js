...
module.exports = async (req, res) => {
        ...
        try {
          ...
          const stripeLineItems = lineItems.map(item => ({
                      price_data: {
                                    ...
                                    product_data: {
                                                    ...
                                                    // Tax code: txcd_99999999 = General Physical Goods
                                                    // Stripe Tax uses this to apply the correct tax rules per state.
                                                    tax_code: 'txcd_99999999',
                                    },
                                    unit_amount: Math.max(0, Math.round(Number(item.amount) || 0)),
                                    // 'exclusive' = tax is added on top of the displayed price (standard US behavior)
          ...
          const session = await stripe.checkout.sessions.create({
                      ...
                      // ✅ STRIPE TAX — calcule automatiquement la taxe selon l'adresse de livraison du client
